...
/**
 * 裂隙传送界面：从裂隙维度返回曾经进入过裂隙的任意维度。
 * <p>
 * 列表只显示维度名称（色块与坐标已按要求移除）；单击条目 → 白框选中；点「传送」→ 传送。
 * 列表可滚动（滚轮 / 拖动右侧滚动条），超出可视区域的行由 scissor 裁掉。
 * <p>
 * 一条记录都没有时不会打开本界面：服务端直接传回世界重生点。
 * <p>
 * 调试：{@code /riftdebug add|remove|clear|list|gui}，塞入 {@code plumestweaks:debug_*} 假记录；
 * 界面检测到调试条目会自动显示调试叠加。
 */
public class RiftTeleportScreen extends Screen {
    ...
    /**
     * 列表行：维度显示名 + 实测像素宽度（判断是否需要横向滚动）。
     * <p>
     * 是可变的：文本宽度要用 {@code Screen.font} 量，而本对象在构造函数里就建好了 ——
     * 那时 {@code font} 还是 null，只能等到渲染时（{@code this.font} 已就绪）惰性补测。
     */
    private static final class Row {
        ...
        /** 惰性量宽；字体还没就绪时返回 -1，调用方按「放得下」处理 */
        int textWidth(net.minecraft.client.gui.Font font) {
    ...
    private void rebuildRows() {
        ...
        for (RiftExitEntry e : exits.ordered()) {
            rows.add(new Row(e, ClientDimensionNames.resolve(e.dimensionId(), e.name())));
        }
    ...
    @Override
    protected void init() {
        // 先量一次文本宽度（此时 font 已就绪），决定哪些行需要横向滚动
        for (Row row : rows) {
            row.textWidth(this.font);
        }

        // 面板高度 = 内容高度 + 一部分富余（富余夹紧，避免列表与按钮之间空一大片）
        ...
        // 列表固定占满一页；富余全部给「列表底 → 按钮」这一段，按钮因此贴近条目
    ...
    public void render(@NotNull GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        // 跑马灯相位按真实帧间隔推进（与帧率无关，也不受游戏刻影响）
        tickMarquee();

        // 背景：贴图 cover 铺满 + 柔纱
        RiftBackdrop.render(graphics, this.width, this.height);

        // 直角面板：纯程序绘制的半透明矩形 + 1px 描边
        ...
        if (rows.isEmpty()) {
            ...
            for (int i = 0; i < rows.size(); i++) {
                ...
                renderRow(graphics, rows.get(i), l.listX(), ry, l.listW(), isSelected, isHovered,
                        i < rows.size() - 1);
            }
        }
     ...
     * 两个必须注意的点：
     * <ul>
     *   <li><b>位移必须是 float</b>。早期版本用 {@code (int)} 取整，位置被量化到整像素，
     *       走动时会一跳一跳，看着就是「卡顿 + 轻微回弹」；</li>
     *   <li><b>时间必须按真实帧间隔累加</b>（见 {@link #tickMarquee()}），
     *       不能用 20Hz 的游戏刻 + partialTick —— 那会把相位锁在游戏刻上。</li>
     * </ul>
     * 行本身已被列表的 scissor 裁掉，超出的部分不会溢出到面板外。
     */
     ...
     * 按真实帧间隔推进跑马灯相位。
     * <p>
     * 用 {@code DeltaTracker#getRealtimeDeltaTicks()}（真实时间流速，不受暂停/冻结影响），
     * 除以 20 得到秒。这样滚动距离只与真实时间成正比，与帧率无关 ——
     * 帧率高更平滑，帧率低也只是步子大一点，不会忽快忽慢或回弹。
     */
    private void tickMarquee() {
        float deltaSeconds = 0f;
        if (this.minecraft != null) {
            deltaSeconds = this.minecraft.getTimer().getRealtimeDeltaTicks() / 20.0f;
        }
        // 卡顿/拖窗口/切出界面时会给出很大的 delta，夹住避免文字瞬移
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        if (maxScroll() > 0f) {
            scrollOffset = Mth.clamp(scrollOffset - (float) scrollY * (ROW_H + ROW_GAP), 0f, maxScroll());
            return true;
        }
    ...
    private void teleport() {
        ...
        PacketDistributor.sendToServer(new RiftTeleportPayload(row.entry().dimensionId()));
        onClose();
    }
