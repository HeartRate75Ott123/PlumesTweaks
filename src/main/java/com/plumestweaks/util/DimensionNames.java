...
/**
 * 维度显示名的通用工具（服务端 / 客户端都可使用，不触碰任何客户端类）。
 * <p>
 ...
 * 客户端侧解析见 {@code com.plumestweaks.client.ClientDimensionNames}。
 */
public final class DimensionNames {

    private DimensionNames() {}

    /** 翻译键（固定格式，内置常见维度；见 lang 文件） */
    ...
    /**
     * 列表左侧色块颜色（维度图标位）。
     * 内置主世界/下界/末地/暮色森林四个取色，其余按 id 哈希取稳定的青蓝色系，
     * 保证同一个维度每次打开界面颜色一致。
     */
    public static int accentColor(String dimensionId) {
        if (dimensionId == null) return 0xFF5AC8C8;
        return switch (dimensionId) {
            case "minecraft:overworld" -> 0xFF6FCF6F;
            case "minecraft:the_nether" -> 0xFFD9603B;
            case "minecraft:the_end" -> 0xFF9B6FD9;
            case "twilightforest:twilight_forest" -> 0xFF4FBF7F;
            default -> {
                ...
                yield 0xFF000000 | hsvToRgb(hue, 0.5f, 0.9f);
            }
        };
    }
